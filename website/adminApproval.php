<?php
  ...
  $lockedAccounts = $user->accountsLocked();
  $alluserData = $user->getAllUserForActivation($status);
    ...
    <div class="row">
      <div class="col-md-10 col-sm-offset-1">
        ...
        <div class="tab-content">
          ...
          <div id="unlockUsers" class="tab-pane fade">
            ...
            <div class="transaction-table">
              <table class="table table-condensed table-hover table-striped">
                ...
                <tbody>
                  ...
                  <?php foreach($lockedAccounts as $lockedData) { ?>
                    <tr>
                      <input type="hidden" class="lockedUserID" value="<?php echo $lockedData['dh_user_id']; ?>">
                      <td class="fullNameText"> <?php echo $lockedData['dh_firstName'].' '.$lockedData['dh_lastName']; ?> </td>
                      <td> <?php echo $lockedData['dh_user_group_name']; ?> </td>
                      <td> <?php echo $lockedData['dh_email_address']; ?> </td>
                      <td> <?php echo $lockedData['dh_date_created']; ?> </td>
                      <td> Locked Account </td>
                      <td> <button class="btn btn-sm btn-warning unlockAccount"> Unlock Account </button> </td>
                    </tr>
                  <?php } ?>
                  <?php } else { ?>
                    <tr>
                      <td colspan="6" style="color:red;text-align: center;"><h2> No Locked Accounts </h2></td>
                    </tr>
                  <?php } ?>
                </tbody>
              </table>
      ...
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <form method="post" role="form" action="commonFunctions.php">
            ...
            <button type="submit" name="approvalAccountAdmin" class="btn btn-success btn-md" > Submit </button>
      ...
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <form method="post" role="form" action="commonFunctions.php">
            ...
            <button type="submit" name="approvalSeminarAdmin" class="btn btn-success btn-md" > Submit </button>
            <button type="button" class="btn btn-primary btn-md" data-dismiss="modal"> Cancel </button>
          </form>
        </div>
      </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <form method="post" role="form" action="commonFunctions.php">
            ...
            <button type="submit" name="approvalTripAdmin" class="btn btn-success btn-md" > Submit </button>
            <button type="button" class="btn btn-primary btn-md" data-dismiss="modal"> Cancel </button>
          </form>
        </div>
      </div>
  ...
  <div id="lockedModal" class="modal fade" role="dialog">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <p> Are you sure you want to <b><span id="lockedmodalText"> </span></b> Account ? </p>
        </div>
        <div class="modal-footer">
          <form method="post" role="form" action="commonFunctions.php">
            <input type="hidden" id="lockedId" name="lockedId">
            <button type="submit" name="unlockAccount" class="btn btn-success btn-md" > Submit </button>
            <button type="button" class="btn btn-primary btn-md" data-dismiss="modal"> Cancel </button>
          </form>
        </div>
      </div>
...
<script>
  ...
  function unlockAccounts(){
    $('.unlockAccount').each(function(){
      var _this = $(this);
      _this.on('click', function(){
        var lockedId = _this.parent().parent().find('.lockedUserID').val();
        var fullName = _this.parent().parent().find('.fullNameText').text();
        $('#lockedId').val(lockedId);
        $('#lockedmodalText').text(' "Unlock" '+ fullName);
